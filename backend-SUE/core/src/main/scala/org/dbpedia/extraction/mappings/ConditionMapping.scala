...
extends Extractor[TemplateNode]
{
  ...
  override def extractWithInfoboxChanges(node : TemplateNode, subjectUri : String, pageContext : PageContext, wikiUpdate: Seq[WikiDML]) : Seq[Quad] =
  {
    ...
    // have to check matches() here. If we are part of a ConditionalMapping, this call of matches()
    // will be redundant, but it's cheap, so it's not a problem.
    if (matches(node)) mapping.extractWithInfoboxChanges(node, subjectUri, pageContext, wikiUpdate)
    else Seq.empty
  }


  override def resolve(input: TemplateNode, subjectUri: String=null, context:PageContext=null, updateSubjectUri: String=null, updatePropertyUri: String, updateValue: String, updateOperation: String): Seq[WikiDML] = {

    if (matches(input)) mapping.resolve(input, subjectUri, context, updateSubjectUri, updatePropertyUri, updateValue, updateOperation)
    else Seq.empty
