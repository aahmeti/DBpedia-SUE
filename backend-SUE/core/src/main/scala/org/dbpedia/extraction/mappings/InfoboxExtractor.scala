...
extends PageNodeExtractor
{
    ...
    def getInfoboxProperties(node : PageNode, predicateUri : String): ArrayBuffer[Tuple4[String, String, String, String]] =
    {
      if(node.title.namespace != Namespace.Main && !ExtractorUtils.titleContainsCommonsMetadata(node.title))
        return null

      val infoboxProperties = new ArrayBuffer[Tuple4[String, String, String, String]]()
        ...
        if ((countExplicitPropertyKeys >= minPropertyCount) && (countExplicitPropertyKeys.toDouble / propertyList.size) > minRatioOfExplicitPropertyKeys) {
          for (property <- propertyList; if (!property.key.forall(_.isDigit))) {
            ...
            for (splitNode <- splitPropertyNodes; (value, datatype) <- extractValue(splitNode)) {
              ...
              if (propertyUri == predicateUri) {
                try

                  infoboxProperties += Tuple4(node.title.decoded.toLowerCase, template.title.decoded.toLowerCase, property.key, value)
  ...
  def extractInfoboxFactsToQuads(node : PageNode, subjectUri : String, pageContext : PageContext) : scala.collection.mutable.HashMap[PropertyNode, Quad] =
  {

    val factsToQuads = new scala.collection.mutable.HashMap[PropertyNode , Quad]

    if(node.title.namespace != Namespace.Main && !ExtractorUtils.titleContainsCommonsMetadata(node.title)) return null
      ...
      if ((countExplicitPropertyKeys >= minPropertyCount) && (countExplicitPropertyKeys.toDouble / propertyList.size) > minRatioOfExplicitPropertyKeys)
      {
        for(property <- propertyList; if (!property.key.forall(_.isDigit))) {
          ...
          for(splitNode <- splitPropertyNodes; (value, datatype) <- extractValue(splitNode))
          {
            ...
            try
            {
              val quad = new Quad(language, DBpediaDatasets.InfoboxProperties, subjectUri, propertyUri, value, splitNode.sourceUri, datatype)
              quads += new Quad(language, DBpediaDatasets.InfoboxProperties, subjectUri, propertyUri, value, splitNode.sourceUri, datatype)

              factsToQuads += (property -> quad) // added
