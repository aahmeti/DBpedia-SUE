...
extends PropertyMapping
{
   ...
   */
    ...
    // try first insert then delete/insert
...
//      // add in the end INSERTs
//      graph ++= writeValue(wikiUpdate.newValue, subjectUri, wikiUpdate.property)
    ...
    override def resolve(input: TemplateNode, originalSubjectUri : String, pageContext : PageContext, updateSubjectUri:String, updatePropertyUri: String, updateValue: String, updateOperation: String): Seq[WikiDML] = {
      ...
      println("Resolving SimplePropertyMapping...")
      val wikiDML = new ArrayBuffer[WikiDML]
      if (updateSubjectUri == null) { // no intermediatenode

        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        // for INSERT we should analyze the mapping
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

        // TODO: here for insert


        for (propertyNode <- input.property(templateProperty) if propertyNode.children.size > 0) {

          //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
          // for Update and Delete it suffices to call extract and do comparison
          //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

          if (ontologyProperty.uri == updatePropertyUri) {

            //TODO: non-functional view definition so far... might delete more infobox data

            val parseResults = parser.parsePropertyNode(propertyNode, !ontologyProperty.isFunctional, transform, valueTransformer)

            // TODO: Treat different datatype values?
            for (parseResult <- selector(parseResults)) {
              val g = parseResult match {

                case value => Seq(
//                  if (updateOperation == "DELETE") {
                    new WikiDML(input.parent.sourceUri,
                      input.title.decoded.toLowerCase.toLowerCase,
                      propertyNode.key,
                      propertyNode.toWikiText.substring(propertyNode.toWikiText.indexOf("=") + 1), // old value
