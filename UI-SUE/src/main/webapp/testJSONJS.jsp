...
<script id="js">
	$(document)
										...
										var req =  $
													.ajax({ // create an AJAX call...
														...
														success : function(
														 	...
														 	$("#dialog").dialog('close');
															//alert('ok! '+ status);
															//alert('ok wikiUpdates! '+ response.wikiUpdates);
															
															var data = response.wikiUpdates; 
															
															var alternativesTriples = $("<div id='alternativesTriples'>");
															var numTriples=0;
															
															var title;
															$.each(data, function(i, TPupdate) { //for each page (triple pattern)
																var accommodations = TPupdate.wikiAccommodations;
																title = TPupdate.wikiTitle; //TODO here we assume we have the same title for all pages
																var triplestab = $("<div id='triplestab-"+(i + 1)+"'>");
																var alternativesDML= $("<div id='alternativesDML'>");
																var tabsvertical = $("<div id='tabsvertical-"+(i + 1)+"'>");
																tabsvertical.addClass("ui-tabs-verticalsmall ui-helper-clearfix");
																var tempOptions= $("<div id='tabsOptionsContent'>");
																
																var numAccommodations=0;
																$.each(accommodations, function(j, accommodation) { 
																	var tab = $("<div id='tabs-"+(j + 1)+"'>");
																	// here we print the tables
																	tab.append(wikiResult());
																	tab.append(startTable());
																	tab.append(printRow(escapeHtml(accommodation.addsWikipedia), "green"));
																	tab.append(printRow(escapeHtml(accommodation.delsWikipedia), "red"));
																	tab.append(closeTable());
																	
																	tab.append(DBpediaResult());
																	tab.append(startTable());
																	tab.append(printRow(escapeHtml(accommodation.addsDBpedia), "green"));
																	tab.append(printRow(escapeHtml(accommodation.delsDBpedia), "red"));
																	tab.append(closeTable());
																	
																	//alert(accommodation.addsWikipedia); 
																	//alert(accommodation.delsWikipedia);
																	//alert(accommodation.addsDBpedia); 
																	//alert(accommodation.delsDBpedia);  
																	tempOptions.append(tab);
																	numAccommodations++;
																});
																
																var tabsOptionsmenu =  $("<div id='tabsOptionsmenu'>");
																var tabsnum =  $("<ul id='tabsnum'>");
																//here add the clicks (line 472)
																for (n = i; n < numAccommodations; n++) {
																	tabsnum.append("<li><a id='tabselect-"+(n+1)+"' href='#tabs-"+(n+1)+"'>OPT#"+(n+1)+"</a></li>");
																}
																
																
																tabsOptionsmenu.append(tabsnum);
																tabsvertical.append(tabsOptionsmenu);
																tabsvertical.append(tempOptions);
																alternativesDML.append(tabsvertical);
																triplestab.append(alternativesDML);
																alternativesTriples.append(triplestab);
															...
															alert(alternativesTriples.html());
															
															// the menu of the triples
															var tabsTriples = $("<ul id='tabsTriples'>");
															for (n = 1; n <= numTriples; n++) {
																tabsTriples.append("<li><a id='tabtriplesselect-"+n+"' href='#triplestab-"+n+"'>TP#"+n+"</a></li>");
															}
															...
															// TODO: the infoboxes with the differences
															$('#infobox_text').html($($.parseHTML(response)).filter("#jsoninfoboxes"));
															
								
															$('#tabsmenuTriples').html(tabsTriples);
															alert($('#tabsmenuTriples').html());
															
															// this works but prevent to reuse in several calls
						
															alert($('#tabscontentTriples').html());
															$('#tabscontentTriples').html(alternativesTriples);
															alert($('#tabscontentTriples').html());
															...
															alert(title); //TODO here we assume we have the same title for all pages
		...
		function startTable() {
			return '<table><tr> <td><h3>ADDs</h3></td><td><h3>DELETEs</h3></td></tr><tr>';
		}
		function closeTable() {
			return "</tr></table>";
		}
		
		function printRow(values,color) {
			// #73AD21
			ret = "<td valign='top'><div style='border-radius: 25px; border: 2px solid " + color
		...
		function wikiResult() {
			return '<h2><img src="img/wikipedia_logo_detail.gif" width="75px" align="middle" style="padding-right: 10px"></img>WIKIPEDIA RESULTS</h2>';
		}
		
		// Print DBpedia results
		function DBpediaResult() {
			return '<h2><img src="img/dbpedia.png" width="75px" align="middle" style="padding-right: 10px"></img>DBPEDIA RESULTS</h2>';
		}
