    ...
    override def extractWithInfoboxChanges(node: TemplateNode, subjectUri: String, pageContext: PageContext, wikiUpdate: Seq[WikiDML]): Seq[Quad] =
    {
        ...
        pageNode.getAnnotation(TemplateMapping.CLASS_ANNOTATION) match
        {
            case None => //So far, no template has been mapped on this page
            {
                ...
                //Extract properties
                graph ++= mappings.flatMap(_.extractWithInfoboxChanges(node, subjectUri, pageContext, wikiUpdate))
            }
            case Some(pageClass) => //This page already has a root template.
            {
                // Depending on the following conditions we create a new "blank node" or append the data to the main resource.
                ...
                //Extract properties
                graph ++= mappings.flatMap(_.extractWithInfoboxChanges(node, instanceUri, pageContext, wikiUpdate))
            }
        }
